/**
 * @file  mod_shared_ptr.hpp
 * @brief mod_shared_ptr is a minimal implementation of smart pointer, a subset of the C++11 std::mod_shared_ptr or boost::mod_shared_ptr.
 *
 ...
 *
 * mod_shared_ptr_count is a container for the allocated pn reference counter.
 */
class mod_shared_ptr_count
{
public:
    mod_shared_ptr_count() :
        pn(NULL)
    {
    }
    mod_shared_ptr_count(const mod_shared_ptr_count& count) :
        pn(count.pn)
    {
    }
    /// @brief Swap method for the copy-and-swap idiom (copy constructor and swap method)
    void swap(mod_shared_ptr_count& lhs) throw() // never throws
    {
        std::swap(pn, lhs.pn);
    }
...
/**
 * @brief minimal implementation of smart pointer, a subset of the C++11 std::mod_shared_ptr or boost::mod_shared_ptr.
 *
 * mod_shared_ptr is a smart pointer retaining ownership of an object through a provided pointer,
...
public:
    ...
    mod_shared_ptr(void) throw() : // never throws
        px(NULL),
        pn()
    {
    }
    /// @brief Constructor with the provided pointer to manage
    explicit mod_shared_ptr(T* p) : // may throw std::bad_alloc
    ...
    template <class U>
    mod_shared_ptr(const mod_shared_ptr<U>& ptr, T* p) :
     //px(p), would be unsafe as acquire() may throw, which would call release() in destructor
       pn(ptr.pn)
    {
       acquire(p);   // may throw std::bad_alloc
    ...
    template <class U>
    mod_shared_ptr(const mod_shared_ptr<U>& ptr) throw() : // never throws (see comment below)
      //px(ptr.px),
        pn(ptr.pn)
    {
        SHARED_ASSERT((NULL == ptr.px) || (0 != ptr.pn.use_count())); // must be coherent : no allocation allowed in this path
        acquire(static_cast<typename mod_shared_ptr<T>::element_type*>(ptr.px));   // will never throw std::bad_alloc
    }
    /// @brief Copy constructor (used by the copy-and-swap idiom)
    mod_shared_ptr(const mod_shared_ptr& ptr) throw() : // never throws (see comment below)
       //px(ptr.px),
        pn(ptr.pn)
    {
    ...
    mod_shared_ptr& operator=(mod_shared_ptr ptr) throw() // never throws
    {
        swap(ptr);
        return *this;
    }
    /// @brief the destructor releases its ownership
    inline ~mod_shared_ptr(void) throw() // never throws
    {
        release();
    }
    ...
    /// @brief Swap method for the copy-and-swap idiom (copy constructor and swap method)
    void swap(mod_shared_ptr& lhs) throw() // never throws
    {
        std::swap(px, lhs.px);
        pn.swap(lhs.pn);
    }
...
private:
    // This allow pointer_cast functions to share the reference counter between different mod_shared_ptr types
    template<class U>
    friend class mod_shared_ptr;

private:
    T*                  px; //!< Native pointer
    mod_shared_ptr_count    pn; //!< Reference counter
};


// comparaison operators
template<class T, class U> inline bool operator==(const mod_shared_ptr<T>& l, const mod_shared_ptr<U>& r) throw() // never throws
{
    return (l.get() == r.get());
}
template<class T, class U> inline bool operator!=(const mod_shared_ptr<T>& l, const mod_shared_ptr<U>& r) throw() // never throws
...
// static cast of mod_shared_ptr
template<class T, class U>
mod_shared_ptr<T> static_pointer_cast(const mod_shared_ptr<U>& ptr) // never throws
{
    return mod_shared_ptr<T>(ptr, static_cast<typename mod_shared_ptr<T>::element_type*>(ptr.get()));
}

// dynamic cast of mod_shared_ptr
template<class T, class U>
mod_shared_ptr<T> dynamic_pointer_cast(const mod_shared_ptr<U>& ptr) // never throws
{
    T* p = dynamic_cast<typename mod_shared_ptr<T>::element_type*>(ptr.get());
    if (NULL != p)
    {
        return mod_shared_ptr<T>(ptr, p);
    }
    else
    {
        return mod_shared_ptr<T>();
    }
}