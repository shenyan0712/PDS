...
采用opencl 1.2的方式
*/
bool dpotrf_v1_cl(clBufferEx<double> &A_buf, int matSize, int tileSize, cldev &cd, int devIdx, clBufferEx<double> &DBIs_buf)
{
	double ret;
	cl_int err;
	int num = 0;
	...
	cl::Kernel *kernel_step1 = cd.get_kernel("chol_v1_step1");
	cl::Kernel *kernel_step2 = cd.get_kernel("chol_v1_step2");
	cl::Kernel *kernel_step3 = cd.get_kernel("chol_v1_step3");
