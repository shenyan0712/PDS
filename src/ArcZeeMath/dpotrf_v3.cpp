...
bool dpotrf_v3_cl(clBufferEx<double> &A_buf, int matSize, int tileSize, cldev &cd, int devIdx, clBufferEx<double> &DBIs_buf)
{
	...
	cl::Kernel *kernel_step1 = cd.get_kernel("chol_v3_step1");
	cl::Kernel *kernel_step2 = cd.get_kernel("chol_v3_step2");
	cl::Kernel *kernel_step3 = cd.get_kernel("chol_v3_step3");
	...
	for (int j = 0; j < cols; j++)
	{
		...
		if (j > 0) {
			...
			cd.get_queue(0).enqueueNDRangeKernel(
				*kernel_step1, cl::NullRange, cl::NDRange((matSize - j*tileSize), tileSize/ VEC_WIDTH),		//globalWorkSize
				cl::NullRange, NULL, NULL); //cl::NDRange(WI_SIZE)
...
bool dpotrf_v3_cl_20(clBufferEx<double> &A_buf, int matSize, int tileSize, cldev &cd, int devIdx, clBufferEx<double> &DBIs_buf)
{
	double ret;
	cl_int err;
	int num = 0;
	...
	num = 0;
	err = kernel_step2->setArg(num++, 1);			// use dev queue or not
	int ks2_ch_num = num++;
	//err = kernel_step2->setArg(num++, j);			//first column
	err = kernel_step2->setArg(num++, tileSize);
	err = kernel_step2->setArg(num++, matSize);
	...
	//call kernel step 2. compute L_jj and inv(L_jj)
	err = kernel_step2->setArg(ks2_ch_num, 0);
	cd.get_queue(0).enqueueNDRangeKernel(
		*kernel_step2, cl::NullRange, cl::NDRange(tileSize, tileSize),		//globalWorkSize
		cl::NDRange(tileSize, tileSize), NULL, NULL); //cl::NDRange(WI_SIZE)
	cd.get_queue(0).finish();
